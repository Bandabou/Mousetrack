...
class TestTrial(Screen, FloatLayout):
	def __init__(self, **kwargs):
		super(TestTrial, self).__init__(**kwargs)
		self.label1 = Label(text="", size_hint=(0.7, 0.6), pos_hint={"right":0.85, "top":0.85}, font_size=sp(18), color=(1,1,1,1))
		self.button = Button(text="NEXT", pos_hint={"right":0.6, "top":0.2}, size_hint=(0.2,0.1), font_size=sp(25), color=(1,1,1,1))
		#time point of leaving this page
		self.leave_time = ""
...
label2_test_trial = "The next items will have an added recommendation\n"\
			  "careful and truthful"



test_trials = TestTrial(name="test_trial")
test_trials.getLabel(label2_test_trial)
...
class DragObj(DragBehavior, Cursor):
	def __init__(self, **kwargs):
		...
		if random.choice(["left", "right"]) == "right":
			self.pos = (Window.width/2-self.width/2), (Window.height/8-self.height/2), #(true is the middle)
		else:
			self.pos = (Window.width/3-self.width/2), (Window.height/8-self.height/2)  #false is left
	...
	def on_touch_down(self, touch):
		if touch.spos[0]*Window.width >= self.x and touch.spos[0]*Window.width <= self.x+self.width and touch.spos[1]*Window.height >= self.y and touch.spos[1]*Window.height <= self.y+self.height:
				...
				#self.parent.parent.update(self) olllldddddddddddddd
	...
	def on_touch_move(self, touch):
		...
		self.coor.append(touch.spos)
		if (self.y > 80): self.parent.parent.update(self) #triggers more than once though
