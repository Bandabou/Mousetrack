...
class InstructionTrial(Screen, FloatLayout):
	...
	def on_press(self, instance):
		self.leave_time = str(datetime.now().time())
		saveData[self.name] = {"leave_time":self.leave_time}
		App.get_running_app().root.current = "control_trial"
...
class cursor_instruct(Screen, FloatLayout):
	...
	def on_press(self, instance):
		self.leave_time = str(datetime.now().time())
		saveData[self.name] = {"leave_time":self.leave_time}
		App.get_running_app().root.current = "trial_down_0"
...
class DragObj(DragBehavior, Cursor):
	...
	def on_touch_up(self, touch):
		...
		if self.end == True:
				...
				if self.name == "trial_up_"+str((len(stimComb2)-1)):
						...
						print("trial_up_" + (str(int(self.name[9:])+1)))
...
class DragObj2(DragBehavior, Cursor):
	def __init__(self, **kwargs):
		...
		if random.choice(["left", "right"]) == "right":
			self.pos = (Window.width/7-self.width/2), (Window.height/8-self.height/2), #(true is the middle)
		else:
	...
	def on_touch_up(self, touch):
		...
		if self.end == True:
				self.leave_time = str(datetime.now().time())
				saveData[self.name] = {"stim":(self.stim1+self.stim2), "coor":self.coor, "time":self.timestamp, "coor2":self.coor2, "time2":self.timestamp2, "events":self.events, "resp":self.response, "leave_time":self.leave_time}
				if self.name == "trial_down_"+str((len(stimComb4)-1)):
						...
						App.get_running_app().root.current = "trial_down_" + str(int(self.name[11:])+1)
						print("trial_down_" + (str(int(self.name[11:])+1)))
...
rec_list = ["The machine recommends left", "The machine recommends right"]
...
#----------------------------
#Creating a mousetrack for the cursor which connects to the trial down trials.
...
class MouseTrackApp(App):
	def build(self):
		...
		for i in range(len(stimComb4)):
			chooser = random.choice(["cursor", "control"])
			if chooser == "control":	
				screen_rec.append(MouseTrack_control(name="trial_down_"+str(i)))
				screen_rec[i].getStim(stimComb4[i][0], stimComb4[i][1])
				screen_rec[i].dragObj.getStim(stimComb4[i][0], stimComb4[i][1])
				ScreenM.add_widget(screen_rec[i])
			elif chooser =="cursor":
				screen_rec.append(MouseTrack_curs(name="trial_down_"+str(i)))
				screen_rec[i].getStim(stimComb4[i][0], stimComb4[i][1])
				screen_rec[i].dragObj.getStim(stimComb4[i][0], stimComb4[i][1])
				ScreenM.add_widget(screen_rec[i])
