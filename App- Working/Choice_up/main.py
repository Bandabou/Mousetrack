...
class rec_instruction(Screen, FloatLayout):
	...
	def on_press(self, instance):
		self.leave_time = str(datetime.now().time())
		saveData[self.name] = {"leave_time":self.leave_time}
		App.get_running_app().root.current = "recommendation_trial"
...
class MouseTrackApp(App):
	def build(self):
		...
		stimComb2 = ['12', '22', '41', '68']
		for i in range(len(stimComb2)):
			screen_trial.append(MouseTrack_curs(name="trial_up_"+str(i)))
